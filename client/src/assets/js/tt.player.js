 ...
 * 20151208 : 发布视频请求成功或失败事件［已删除］
 * 20151213 : 根据视频请求结果设置ttVideoStatus
 * 20160318 : 审核后台针对播放器优化样式
 * 20151208 : 发布视频请求成功或失败事件［重新启用］,添加video标签的id
 **/
...
//获取视窗宽度
    ...
    function data2video() {
        ...
        for (var i = 0; i < containers.length; i++) {
           ...
           // if(containers[i].getAttribute('tt-datainited') == null) {
                ...
                //var urlstr = location.protocol + '//i.snssdk.com/video/urls/1/toutiao/mp4/'+ id;
                var urlstr = '//i.snssdk.com/video/urls/1/toutiao/mp4/'+ id;
                jsonp(urlstr,function (result) {
                    ...
                    if (result.code == 0 && status == 10){
                        ...
                        var decodeUrl = base64decode(originUrl).replace('http://', '//');
                        j++;
                        //判断ie8浏览器特殊处理poster
                        ...
                        for (var w = 0; w < containerArr.length; w++) {
                            if (containerArr[w].url) containerArr[w].url = containerArr[w].url.replace('http://', '//');
                            if (result.data.video_id && containerArr[w].id == result.data.video_id) {
                                ...
                                if(/iphone|ipad/i.test(navigator.userAgent)){
                                    template = '<video src="' + decodeUrl + '" type="video/mp4" controls="" id="'+id+'" poster="' + containerArr[w].url + '" preload="none" webkit-playsinline playsinline style="width:100%;"></video>';
                                    containerArr[w].box.style.cssText += 'margin:0 auto;';

                                } else if (/Android/i.test(navigator.userAgent)){
                                    template = '<video src="' + decodeUrl + '" type="video/mp4" controls="" id="'+id+'" poster="' + containerArr[w].url + '" preload="none" style="width:100%;"></video>';
                                    containerArr[w].box.style.cssText += 'margin:0 auto;';
                                    ...
                                    if(containerArr[w].box.getAttribute('tt-autoplay') == "true" && location.hostname == "admin.bytedance.com"){
                                        template = '<video class="reviewTab" src="' + decodeUrl + '" type="video/mp4" controls="" poster="' + containerArr[w].url + '" preload="none" autoplay="autoplay" style="width:100%;height:100%;"></video>';
                                ...
                                if(containerArr[w].box.getAttribute('tt-autoplay') == "true"){
                                    containerArr[w].box.getElementsByTagName("video")[0].setAttribute("autoplay","autoplay")
                                }
                                // 视频请求成功,ttVideoStatus设置为1
                                window.ttVideoStatus = 1;
                                // 触发视频请求成功事件
                        ...
                        // pc端添加后台播放异常显示处理
                        for (var w = 0; w < containerArr.length; w++) {
                            if((location.hostname == "admin.bytedance.com" || location.hostname == "mp.toutiao.com" || location.hostname == "www.kuaima.im" || location.hostname == "kuaima.toutiao.com") && status != 10){
                                var statusObj = {
                                    '20':'转码失败',
                                    '30':'转码进行中',
                                    '40':'视频id不存在',
                                ...
                                template ='<span style="display: inline-block;height: 100%;width: 100%;line-height: '+ offsetWidth*v_ratio+'px;background-color: rgba(0, 0, 0,0.9);color: #fff;">'+ statusDesc?(result.message + '：' + statusDesc):result.message +'</span>';
                            }
                            containerArr[w].box.innerHTML = template;
                            // 视频请求失败,ttVideoStatus设置为2
                            window.ttVideoStatus = 2;
                            // 触发视频请求失败事件
                            window.listener && listener.trigger('com.toutiao.m.videoContent','video-loaded-failed');
                            containerArr[w].box.style.cssText+= 'width:'+ offsetWidth +'px;height:'+ offsetWidth*v_ratio+'px;margin:0 auto;text-align: center;';
                            insertLog();
                        }
                    }
                });
    ...
    function videoController (){
        if(location.hostname == "admin.bytedance.com"){
            ...
            element.innerHTML = 'video::-webkit-media-controls-panel {opacity:0.4 !important;}video::-webkit-media-controls-current-time-display {display:inline-block !important;}video::-webkit-media-controls-time-remaining-display {display:inline-block !important;}';
    ...
    function hackVideo () {
        ...
        hm.src = "//s0.pstatp.com/html5media1.1.8/html5media.min.js";
    ...
    function insertLog() {
        var hm = document.createElement("script");
        hm.src = "//s0.pstatp.com/tt_player/tt.player.log.js";
        var s = document.getElementsByTagName("script")[0];
        s.parentNode.insertBefore(hm, s);
    }

    window.data2video = data2video;
